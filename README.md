...
- **画笔**涂抹或**框选**要修改的区域 → 输入"改成什么" → 发送
- 顶部可**切换重绘模型**：默认**沿用这张图原本的模型**，也可随时改成别的（比如 Seedream 改的图，可以用 GPT 再修；反之亦然）
...
| **BytePlus / Seedream** | prompt 里注入归一化坐标 | 格式 `Image 1 x1 y1 x2 y2`，坐标范围 **0-999**（左上 0,0 / 右下 999,999），并声明 keep unchanged |

> 图片超过 2048px 时会自动降采样（图与遮罩同尺寸重编码），避免超大请求体经代理传输时出错。
